...
public class Core {
     ...
     * 一般alpha + beta = 1, 比如alpha = 1.5, beta = -0.5,增加图像的对比度
     * @param src1  输入的第一个Mat对象
     * @param alpha     混合时第一个Mat对象所占的权重大小
     * @param src2  输入的第二个Mat对象(一般是纯黑色的背景图像)
     * @param beta  混合时第二个Mat对象所占的权重大小
     * @param gamma 混合之后是否进行亮度校正  默认值为0，为正数时增加亮度
     * @param dst   输出权重叠加之后的Mat对象
     */
    ...
    /**
     * 与,与操作对两张图像混合之后的输出图像有降低混合图像亮度的效果，会让输出的像素小于等于对应位置的任意一张输入图像的像素值
     * @param src1
     * @param src2
     * @param dst
     */
    ...
    /**
     * 取反，取反操作对二值图像是一个常见操作，有时候我们需要先进行取反操作，然后再对图像进行更好地分析
     * @param src
     * @param dst
     */
    ...
    /**
     * 或，或操作对两张图像混合之后的输出图像有强化混合图像亮度的效果，会让输出的像素大于等于对应位置的任意一张输入图像的像素值
     * @param src1
     * @param src2
     * @param dst
     */
     ...
     * @param src
     * @param mean  各个通道的均值
     * @param stddev    各个通道的方差（标准方差越小,说明图像各个像素的差异越小,图像本身携带的有效信息越少。可通过此结论提取和过滤质量不高的图像）
     * @param mask      只有当mask对应位置的像素值不为零时，src相同位置的像素点才参与计算均值与方差
     */
     ...
     * @param src
     * @param mean  各个通道的均值
     * @param stddev    各个通道的方差（标准方差越小,说明图像各个像素的差异越小,图像本身携带的有效信息越少。）
     */
     ...
     * @param src   输入的图像
     * @param dst   输出的图像
     * @param alpha 归一化到指定范围的低值
     * @param beta  归一化到指定范围的高值
     * @param norm_type 比如Core.NORM_MINMAX,最小与最大值归一化方法：dst = [(x - min) / (max - min)] * (beta - alpha) + alpha, x为src的像素值
     *                  如果有负值，可以通过convertScaleAbs来对负值求绝对值
     * @param dtype 输出的dst图像类型，默认为-1，表示与输入图像的src相同类型
     * @param mask  表示遮罩层，默认为Mat()
     */
